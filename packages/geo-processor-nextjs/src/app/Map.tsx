...
export default function Map({ points, result, onMapClick, onRemovePoint }: MapProps) {
  return (
        ...
        {result && (
          <>
            <Marker position={[result.centroid.lat, result.centroid.lng]} icon={customIcon}>
              <Popup>
                Centroide: Lat {result.centroid.lat.toFixed(4)}, Lng {result.centroid.lng.toFixed(4)}
              </Popup>
            </Marker>
            <Polyline
              positions={[
